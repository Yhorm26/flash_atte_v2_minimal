...
__global__
void forward_kernel_5(mykernelParamType2 param) {
    int tx = threadIdx.x;
    ...
    size_t q_offset_tx  = tx * load_Q_num;
    size_t kv_offset_tx = tx * load_KV_num;

    #pragma unroll
    for (int k = 0; k < load_Q_num / 8; k++){
        CP_ASYNC_CG(__cvta_generic_to_shared(&Qj[q_offset_tx+k*8]), &Q[q_offset_tx+k*8], 16);
    }
    #pragma unroll
